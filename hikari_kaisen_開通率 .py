## 光回線の開通率を分析するための算出スクリプト。
## 申込から開通までのリードタイムも確認できるようにし、開通率が悪い、もしくは遅いという問題を可視化できるようにしたもの。
## 利用不可物件からの申込や提供エリア外での申込が多く、開通率が上昇しにくいことが判明した。
## このデータをもとに、申込の間口を広げたままにするか、開通率の上昇を重視するかを検討しLPや申込フォームの改修を進めている。
