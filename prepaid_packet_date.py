## プリペイドWi-Fiの通信状況を分析したスクリプト。
## 開通日から終了日までの日次の通信状況と、再チャージ記録と、容量超過を可視化したもの。
## また、利用開始日をN日とし、利用期間の中で利用量のピーク時も確認することが可能。
## 複数あるプランの中で、どういった使われ方をされているかを分析し、日数と容量の組み合わせに応じて利用量に相関関係があることが判明した。
## この相関関係から近似曲線を求め、新規プラン作成の際に、想定利用量と原価を算出できるように設計し、利用状況の分析だけでなく、新規プランの提案にも活用している。
